...
let camRed = () => src(s0).color(1,0,0)
let camGreen = () => src(s0).color(0,1,0)
let camBlue = () => src(s0).color(0,0,1)

var sine = () => Math.abs(1.01 * Math.sin(time * -0.05) + 0);
...
// GRID
let gridSize = 500 // 120 through 160 are weird. 150 is null // below 100 needs height adjustment
let gridTime = 0.000 
let threshVal = 0.020
let grid = () => 
	osc(gridSize, gridTime, 0).thresh(threshVal).rotate(0)
	.mult(osc(gridSize-150, gridTime, 0).thresh(threshVal).rotate(1.571)) // 
// .modulate(osc(5, gridTime, 0))

let gridOsc1 = () => Math.abs(0.01 * Math.sin(time * 0.5) + 6);
let gridOsc2 = () => Math.abs(0.1 * Math.sin(time * 0.5) + -1.57);
let modGrid = () => 
	osc(gridSize, gridTime, 0).thresh(threshVal).rotate(gridOsc1)
	.mult(osc(gridSize-150, gridTime, 0).thresh(threshVal).rotate(gridOsc2))
// .modulate(osc(5, gridTime, 0))
// .colorama(5)
// .color(1.5, 0.2, sine)
...
cam()
	.modulate(camRed(), ()=>cc[51])
	.modulate(camGreen(), ()=>cc[52])
	.modulate(src(o1), ()=>cc[53])
	.layer(src(o2).luma(()=>cc[56]))
	// .layer(cam().modulate(camBlue()), gridOsc1)
	// .layer(shape(4, ()=>cc[21]))
	.out(o1)

cam()
	// .thresh(0.03).color(1,0,0).invert()
	// .colorama(()=>cc[54])
	.modulate(src(o1), ()=>cc[55]*.1)
	.out(o2)
