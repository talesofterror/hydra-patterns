...
// s0.initImage("images/city.50p.fs.16color.gif")
s0.initVideo("images/city.50p.fs.16color.mp4")
...
// s3.initImage("images/sky.50p.fs.16color.gif")
s3.initVideo("images/sky.50p.fs.16color.mp4")
...
var sineAbs = () => Math.abs(0.02 * Math.sin(time * sineSpeed) + 0.05);
setInterval(() => console.log(sineAbs()), 1000)
var sineAbsFast= () => Math.abs(0.02 * Math.sin(time * sineSpeed2) + 0.5);
...
src(s0)
  ...
  .add(src(s2))
  // .mult(s0)
  .add(src(s3).scrollX(0.02, -0.002))
  .add(green(o3), 1) // WINDOWS
  .modulate(redCityStripes().scrollX(0.045, 0.00), 0.05)
  .mult(o1, sineAbs)
  .modulate(osc(3, 0.02, 0).rotate(1.6), sineAbs)
  // .scrollX(0.5, -0.01)
  // .add(o1, 1)
  .mult(src(o2).invert(), sineAbs)
  .modulate(src(o0).invert(), 0.001)
  .sub(o2, 0.01)
.out(o0)

src(s1)
.sub(src(s2))
.mult(red(o1), sineAbs2)
.diff(src(s1), sineAbsFast)
.modulate(osc(200, 0.2, 0), 0.02)
.out(o1)
